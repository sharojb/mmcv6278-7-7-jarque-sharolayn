  ...
  describe('e-commerce api', () => {
    describe('authentication', () => {
      ...
      it('Logging out should NOT allow access to cart page', async () => {
        ...
        expect(loginResult.status).to.eq(302)
        expect(loginResult.headers.location).to.eq('/')
        const cartResultLoggedIn = await agent
          .get('/cart')
        ...
        const cartResultLoggedOut = await agent
          .get('/cart')
    ...
    describe('cart', () => {
      ...
      it('PUT /api/cart/:id should change cart quantity', async () => {
        ...
        await db.query(`INSERT INTO cart (inventory_id, quantity, user_id) VALUES (3,3,1)`)
        const [[addedItem]] = await db.query(`SELECT * FROM cart WHERE inventory_id=3 AND user_id=1`)
        const res = await agent
          .put(`/api/cart/${addedItem.id}`)
          .send(itemToChange)
        const [[changedItem]] = await db.query(`SELECT * FROM cart WHERE inventory_id=3 AND user_id=1`)
      ...
      it('PUT /api/cart/:id should return 404 if cart item does not exist', async () => {
        ...
        const res = await agent
          ...
          .send(itemToChange)
        expect(res.status).to.eq(404)
        const [[unchangedItem]] = await db.query(`SELECT * FROM cart WHERE inventory_id=3 AND user_id=1`)
      ...
      it('DELETE /api/cart/:id should remove cart item', async () => {
        ...
        const [[deletedItem]] = await db.query(`SELECT * FROM cart WHERE inventory_id=3 AND user_id=1`)
      ...
      it('DELETE /api/cart/:id should return 404 if cart item does not exist', async () => {
        ...
        const res = await agent
          .delete(`/api/cart/99`)
        expect(res.status).to.eq(404)
        const [[undeletedItem]] = await db.query(`SELECT * FROM cart WHERE inventory_id=3 AND user_id=1`)
        expect(addedItem).to.deep.eq(undeletedItem)
      })
      it('DELETE /api/cart/ should empty cart', async () => {
        const res = await agent
          .delete(`/api/cart`)
        const [[{count}]] = await db.query(`SELECT count(*) as count FROM cart WHERE user_id=1`)
  ...
  describe('e-commerce front-end routes', () => {
    describe('GET / - index/inventory page', () => {
      ...
      it('should show login link instead of cart count if not logged in', async () => {
        ...
        expect(document.querySelector('header').textContent.match(/cart/i)).to.be.null
        expect(document.querySelector('header').textContent.match(/login/i)).to.exist
      })
      it('should show cart count if logged in', async () => {
        ...
        expect(res.headers['content-type']).to.include('html')
        const { window: { document } } = new JSDOM(res.text)
        const [[{count}]] = await db.query(`SELECT SUM(quantity) AS count FROM cart WHERE user_id=1;`)
